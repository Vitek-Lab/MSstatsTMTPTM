...
**!! todo : Protein_4_Peptide_455 for the first row is not matched to what I can see in html. I have the first row with Protein12_S703 for protein and Peptide_491 for peptidesequence. **
...
After loading in the input data, the next step is to use the proteinSummarization function from MSstatsTMT. This provides the summarized dataset needed to model the protein/PTM abundance. The function will summarize the Protein dataset up to the protein level and will summarize the PTM dataset up to the PTM level. The different summarizations are caused by adding the PTM site into the `ProteinName` field. **!! todo : add the sentences-The summarization for ptm and protein datasets should be done separately. The details about normalization and imputation option in proteinSummarization is available in xxx (point to MSstatsTMT package).**
...
After the two datasets are summarized, they**!!todo: replace they can ->  both the summarized PTM and protein abundance should ?** be used in the modeling function `groupComparisonTMTPTM`.
...
**!!todo : You can define the contrast matrix. Here is the example of contrast.matrix (It will be good to show how example.comparisons looks like)**
``` {r,  message=FALSE, warning=FALSE}
...
The modeling function will return a list consisting of three dataframes. One each for the PTM, Protein, and Adjusted PTM models. **!!todo : change this sentence like : One each for the PTM-level, Protein-level, and adjusted PTM-level group comparisno result? **

### 1.4 Example Volcano Plot

The models from the `groupComparisonTMTPTM` function can be used in the model visualization function, `groupComparisonPlots`, from the base MSstats. Below is a Volcano Plot for the Adjusted PTM model.**!!todo : Note: the input for groupComparisonPlots should be one data.frame from output of groupComparisonTMTPTM.**
...
model_df
```
**!!todo: in the html, row names are not shown. so confuse what the first two rows for.**
...
In this example the PTM model originally showed a large log2FC of 0.242, however most of this **!!todo :log2 FC? **FC was due to the global protein abundance change which was 0.356. Once adjusted, the PTM shows a much smaller FC of -0.114. Without adjustment a potentially incorrect inference could have been drawn for this PTM.
